...
export function cleanExpression(text) {
  return String(text || "")
    ...
    .replace(/\s+/g, "")
    .replace(/[^0-9+\-*/().]/g, "")
    // Convierte multiplicaciones implicitas en simbolos explicitos.
    .replace(/(\d|\))(?=\()/g, "$1*")
    .replace(/\)(?=\d)/g, ")*");
}
...
export function parseExpression(expression) {
  ...
  if (position < tokens.length) {
    throw new Error("Hay simbolos sobrantes.");
  }
...
// Presenta resultados enteros y decimales de forma limpia.
